...
class DiscConvNet(nn.Module):
    ...
    def __init__(self, image_shape, n_ch = 1, feat_vec = [32, 64, 128, 256], strides = [2, 2, 2, 1]):
        self.conv1 = DiscConvLayer(n_ch, feat_vec[0])
        self.conv2 = DiscConvLayer(feat_vec[0], feat_vec[1])
        self.conv3 = DiscConvLayer(feat_vec[1], feat_vec[2])
        self.conv4 = DiscConvLayer(feat_vec[2], feat_vec[3])
        self.flatdim = (image_shape[0]/16)*(image_shape[1]/16)*(image_shape[2]/16)*feat_vec[3]
        self.dense = nn.Linear(flatdim, 1)
